...
if (seatUsers.count <= 2) { // Increased threshold to ensure we add our new rich data
  ...
  const pcId = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('Workstation-01', 'DELL-P87W-01', 'PC', 'Laptop', 'En service', locParis, supHardware, userIT, pcSpecs).lastInsertRowid;
  ...
  const pc2Id = db.prepare(`
    INSERT INTO assets (label, serial, type, subtype, status, location_id, supplier_id, assigned_user_id, specs)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run('MacBook-Rh-02', 'APL-QC88-99', 'PC', 'Laptop', 'En service', locParis, supHardware, userHR, pc2Specs).lastInsertRowid;

  // 5. Assets: Peripherals (Children of PC1)
  const monId = db.prepare(`
    INSERT INTO assets (label, serial, type, status, location_id, supplier_id, parent_asset_id)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run('Écran Dell Ultrasharp 27', 'MON-DS27-101', 'Écran', 'En service', locParis, supHardware, pcId).lastInsertRowid;

  db.prepare(`
    INSERT INTO assets (label, serial, type, status, location_id, supplier_id, parent_asset_id)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run('Souris MX Master 3S', 'LOGI-MS78-9', 'Souris', 'En service', locParis, supPeriph, pcId).run();

  db.prepare(`
    INSERT INTO assets (label, serial, type, status, location_id, supplier_id, parent_asset_id)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run('Clavier Apple Magic', 'APL-KB-442', 'Clavier', 'En service', locParis, supPeriph, pc2Id).run();
...
async function startServer() {
  ...
  app.get('/api/stats', (req, res) => {
    ...
    // Upcoming license expirations (next 3 months)
    const upcomingExpirations = db.prepare(`
      SELECT label as name, end_date as date, 'License' as type
      FROM licenses 
      WHERE end_date IS NOT NULL AND end_date > date('now') AND end_date <= date('now', '+3 months')
      UNION ALL
  ...
  app.post('/api/assets', (req, res) => {
    const { label, serial, inventory_number, type, subtype, status, location_id, supplier_id, assigned_user_id, parent_asset_id, specs } = req.body;
    const info = db.prepare(`
      INSERT INTO assets (label, serial, inventory_number, type, subtype, status, location_id, supplier_id, assigned_user_id, parent_asset_id, specs)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(label, serial, inventory_number, type, subtype, status || 'Stock', location_id, supplier_id, assigned_user_id, parent_asset_id, specs);
  ...
  app.put('/api/assets/:id', (req, res) => {
    ...
    db.prepare(`
      UPDATE assets 
      SET label = ?, serial = ?, inventory_number = ?, type = ?, subtype = ?, status = ?, location_id = ?, supplier_id = ?, assigned_user_id = ?, parent_asset_id = ?, specs = ?, updated_at = CURRENT_TIMESTAMP
      WHERE id = ?
    `).run(label, serial, inventory_number, type, subtype, status, location_id, supplier_id, assigned_user_id, parent_asset_id, specs, id);
  ...
  // Migration for parent_asset_id in assets
  try {
    const tableInfo = db.prepare("PRAGMA table_info(assets)").all() as any[];
    const hasParentAssetId = tableInfo.some(col => col.name === 'parent_asset_id');
    if (!hasParentAssetId) {
      db.exec("ALTER TABLE assets ADD COLUMN parent_asset_id INTEGER REFERENCES assets(id)");
    }
  } catch (err) {
    console.error("Migration error (assets parent_asset_id):", err);
  }
  ...
  // Migration for inventory_number in assets
  try {
    const tableInfo = db.prepare("PRAGMA table_info(assets)").all() as any[];
    const hasInv = tableInfo.some(col => col.name === 'inventory_number');
    if (!hasInv) {
      db.exec("ALTER TABLE assets ADD COLUMN inventory_number TEXT");
    }
  } catch (err) {
    console.error("Migration error (assets inventory_number):", err);
  }

  // Migration for reference in contracts
  try {
    const tableInfo = db.prepare("PRAGMA table_info(contracts)").all() as any[];
    const hasReference = tableInfo.some(col => col.name === 'reference');
    if (!hasReference) {
      db.exec("ALTER TABLE contracts ADD COLUMN reference TEXT");
    }
  } catch (err) {
    console.error("Migration error (contracts reference):", err);
  }

  // Migration for supplier_id in licenses
  try {
    const tableInfo = db.prepare("PRAGMA table_info(licenses)").all() as any[];
    const hasSupplierId = tableInfo.some(col => col.name === 'supplier_id');
    if (!hasSupplierId) {
      db.exec("ALTER TABLE licenses ADD COLUMN supplier_id INTEGER REFERENCES suppliers(id)");
    }
  } catch (err) {
    console.error("Migration error (licenses):", err);
  }
