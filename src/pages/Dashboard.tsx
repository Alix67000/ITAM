    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <motion.div 
          ...
          className={cn(theme.card, "p-5 hover:shadow-xl hover:border-indigo-100 transition-all group relative cursor-pointer")}
        >
          ...
          <div className="flex items-end justify-between mt-3 relative">
            <h3 className="text-3xl font-black text-slate-900 tracking-tighter">{stats.counts.assets}</h3>
            <div className="w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center shadow-lg shadow-indigo-200">
              <Laptop className="w-5 h-5" />
            </div>
          </div>
          <p className="text-[10px] text-indigo-600 font-bold mt-4 flex items-center gap-1 uppercase tracking-tighter whitespace-nowrap">
        ...
        >
          ...
          <div className="flex items-end justify-between mt-3 relative">
            <h3 className="text-3xl font-black text-slate-900 tracking-tighter">{stats.counts.users}</h3>
            <div className="w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center shadow-lg shadow-indigo-200">
              <Users className="w-5 h-5" />
            </div>
          </div>
          <p className="text-[10px] text-slate-500 font-bold mt-4 uppercase tracking-tighter">Profils collaborateur</p>
        </motion.div>

        <motion.div 
          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} 
          onClick={() => navigate('/locations')}
          className={cn(theme.card, "p-5 hover:shadow-xl hover:border-slate-300 transition-all group relative cursor-pointer")}
        >
          ...
          <div className="flex items-end justify-between mt-3 relative">
            <h3 className="text-3xl font-black text-slate-900 tracking-tighter">{stats.counts.locations}</h3>
            <div className="w-10 h-10 bg-slate-800 text-white rounded-xl flex items-center justify-center shadow-lg shadow-slate-200">
              <MapPin className="w-5 h-5" />
            </div>
          </div>
          <p className="text-[10px] text-slate-500 font-bold mt-4 uppercase tracking-tighter">Sites & Services</p>
        </motion.div>

        <motion.div 
          initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} 
          onClick={() => navigate('/contracts')}
          className={cn(theme.card, "p-5 hover:shadow-xl hover:border-emerald-100 transition-all group relative cursor-pointer")}
        >
          ...
          <div className="flex items-end justify-between mt-3 relative">
            <h3 className="text-3xl font-black text-slate-900 tracking-tighter">{stats.counts.contracts}</h3>
          ...
          <p className="text-[10px] text-emerald-600 font-bold mt-4 uppercase tracking-tighter">Engagements actifs</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className={cn(theme.card, "p-5 hover:shadow-xl hover:border-blue-100 transition-all group relative")}>
          ...
          <p className="text-[10px] text-blue-600 font-bold mt-4 uppercase tracking-tighter">Investissement total</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className={cn(theme.card, "p-5 hover:shadow-xl hover:border-emerald-100 transition-all group relative")}>
           <div className="absolute top-0 right-0 w-24 h-24 bg-emerald-50/50 rounded-full -mr-12 -mt-12 group-hover:scale-110 transition-transform" />
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest relative">Taux Garantie</p>
          <div className="flex items-end justify-between mt-3 relative">
            <h3 className="text-3xl font-black text-slate-900 tracking-tighter">{Math.round(stats.counts.warrantyPercent)} <span className="text-xl">%</span></h3>
            <div className="w-10 h-10 bg-emerald-600 text-white rounded-xl flex items-center justify-center shadow-lg shadow-emerald-200">
              <ShieldAlert className="w-5 h-5" />
            </div>
          </div>
          <p className="text-[10px] text-emerald-600 font-bold mt-4 uppercase tracking-tighter">Matériel converti</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className={cn(theme.card, "p-5 hover:shadow-xl hover:border-amber-100 transition-all group relative")}>
          ...
          <div className="flex items-end justify-between mt-3 relative">
            <h3 className="text-3xl font-black text-slate-900 tracking-tighter">{stats.counts.averageAgeYears.toFixed(1)} <span className="text-xl">Ans</span></h3>
          ...
          <p className="text-[10px] text-amber-600 font-bold mt-4 uppercase tracking-tighter">Obsolescence moyenne</p>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-red-50 p-5 rounded-2xl border border-red-100 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative">
          ...
          <div className="flex items-end justify-between mt-3 relative">
            <h3 className="text-3xl font-black text-red-700 tracking-tighter">{stats.counts.broken}</h3>
            <div className="w-10 h-10 bg-red-600 text-white rounded-xl flex items-center justify-center shadow-lg shadow-red-200 animate-pulse">
              <AlertCircle className="w-5 h-5" />
            </div>
          </div>
          <p className="text-[10px] text-red-600 font-bold mt-4 uppercase tracking-tighter">Matériels en panne</p>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Trend Area Chart */}
        <div className="lg:col-span-8 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-[350px]">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
               <div className="flex items-center gap-1.5 px-2 py-1 bg-blue-50 text-blue-600 rounded-lg text-[9px] font-bold uppercase border border-blue-100">
            ...
            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
              <AreaChart data={stats.charts.trends} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip 
                  ...
                  labelStyle={{ fontWeight: 'black', fontSize: '11px', color: '#0f172a', marginBottom: '2px' }}
                  itemStyle={{ fontWeight: 'bold', fontSize: '11px' }}
                />
                <Area type="monotone" dataKey="count" name="Total" stroke="#3b82f6" strokeWidth={3} fillOpacity={1} fill="url(#colorCount)" dot={{ r: 3, fill: '#3b82f6', strokeWidth: 2, stroke: '#fff' }} activeDot={{ r: 5, strokeWidth: 0 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col min-h-[350px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
          <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
            ...
            <button className="text-[10px] font-black text-blue-600 hover:bg-blue-50 px-3 py-1.5 rounded-lg transition-all border border-blue-100 uppercase tracking-widest bg-white shadow-sm">
            ...
            <div className="space-y-1">
              {stats.recentEvents.map((event, idx) => (
                <motion.div 
                  ...
                  className="flex items-center gap-4 p-3 hover:bg-slate-50 rounded-xl transition-all cursor-pointer group"
                >
                  <div className={cn(
                    ...
                    {event.action === 'Création' ? <Plus className="w-4 h-4" /> :
                     event.action === 'Panne' ? <AlertCircle className="w-4 h-4" /> :
                     <Edit2 className="w-4 h-4" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <h5 className="text-sm font-bold text-slate-900 truncate tracking-tight">{event.description}</h5>
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{new Date(event.created_at).toLocaleDateString('fr-FR')}</span>
                    </div>
                    <div className="flex items-center gap-2">
                       <span className="text-[9px] px-2 py-0.5 bg-slate-100 text-slate-500 rounded border border-slate-200/60 font-bold uppercase tracking-tight">
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 overflow-hidden relative group">
            ...
            <h4 className="text-[10px] font-black uppercase tracking-widest text-amber-600 mb-4 flex items-center gap-2 relative">
              <ShieldAlert className="w-4 h-4" /> Veille préventive
            </h4>
            
            <div className="space-y-3 relative">
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest border-b border-slate-50 pb-2">Échéances Proches</p>
              ...
              {stats.upcomingExpirations.map((exp, i) => {
                ...
                return (
                  <div key={i} 
                    ...
                    className={cn(
                    "group/item flex items-start gap-3 p-3 rounded-xl transition-all cursor-pointer border",
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between mb-0.5">
                        <div className="text-[10px] font-black text-slate-900 truncate pr-2 tracking-tight uppercase">{exp.name}</div>
                        <div className={cn(
                      ...
                      <div className="text-[9px] text-slate-500 mt-0.5 flex items-center gap-1.5 font-medium">
              ...
              {stats.upcomingExpirations.length === 0 && (
                <div className="text-[10px] text-slate-400 font-bold italic py-6 text-center uppercase tracking-widest">
              ...
              <button 
                ...
                className="w-full mt-3 py-3 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-sm shadow-amber-200 transition-all active:scale-95"
              >
          ...
          {/* Quick Support Card */}
          <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-6 rounded-2xl shadow-xl shadow-blue-200 text-white group relative overflow-hidden">
            ...
            <h4 className="text-[10px] font-black uppercase tracking-widest opacity-80 mb-2">Besoin d'aide ?</h4>
            <h3 className="text-lg font-black mb-3 tracking-tight">Support IT Interne</h3>
            <p className="text-[11px] opacity-80 mb-5 leading-relaxed font-medium">Un incident technique ? Un renouvellement nécessaire ? Nos équipes sont là.</p>
            <button className="flex items-center justify-center w-full gap-2 bg-white text-blue-600 px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-all shadow-sm">
              Ouvrir un ticket
            </button>
          </div>
        </div>
